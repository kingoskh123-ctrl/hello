...
# ==========================================================
# BOT CONSTANT SETTINGS (R_100 | Rise/Fall | Immediate Reversed Martingale x2.2 | 5 Ticks)
# ==========================================================
...
active_processes = {}
active_ws = {}
is_contract_open = {} # 🟢 تم إعادته من الكود القديم لمعالجة مشكلة القفل
PROCESS_LOCK = Lock()
TRADE_LOCK = Lock() 
...
DEFAULT_SESSION_STATE = {
    ...
    "open_contract_id": None, # 🟢 تم إعادته للـ Recovery
}
...
def stop_bot(email, clear_data=True, stop_reason="Stopped Manually"):
    global active_processes, active_ws, is_contract_open
...
# 🟢 تم تحديث هذه الدالة لوضع القفل العام (is_contract_open) فور إرسال الطلب
def send_rise_fall_order(email, stake, currency, direction):
    global active_ws, DURATION, DURATION_UNIT, SYMBOL, is_contract_open
    ...
    try:
        ...
        # 🟢 نقطة حاسمة: وضع القفل العام هنا (كما في الكود القديم)
...
def apply_martingale_logic(email, entry_direction):
    ...
    current_data['open_contract_id'] = None # 🟢 تحديث متغير الاسترداد
    ...
    # ------------------- Loss State -------------------
        ...
        if send_rise_fall_order(email, current_data['current_stake'], current_data['currency'], reversed_direction):
            # القفل تم وضعه داخل send_rise_fall_order
            pass 
        else:
...
# 🟢 تم تحديث هذه الدالة لإلغاء القفل العام (is_contract_open)
def handle_contract_settlement(email, contract_id, profit_loss):
    global is_contract_open
    
    current_data = get_session_data(email)
    
    if contract_id not in current_data.get('open_contract_ids', []):
        return
    ...
    # 🟢 إزالة contract_id من سجل الاسترداد
    ...
    if not current_data['open_contract_ids'] and len(current_data['contract_profits']) == 1:
        ...
        # 🟢 إلغاء القفل العام (النقطة الأهم بعد الانتهاء من الصفقة)
...
def bot_core_logic(email, token, stake, tp, currency, account_type):
    ...
    global active_ws, TICK_SAMPLE_SIZE, is_contract_open

    active_ws[email] = None
    is_contract_open[email] = False # تهيئة القفل
    ...
    try:
        while True:
            ...
            def on_open_wrapper(ws_app):
                ...
                # 🟢 منطق استرداد العقد (Recovery) باستخدام open_contract_id
                contract_id = current_data.get('open_contract_id')
                if contract_id:
                    ...
                    print(f"🔍 [RECOVERY CHECK] Found contract pending settlement. RE-SUBSCRIBING: {contract_id[:8]}...")
                
            # 🟢 دالة on_message_wrapper المحدثة لاستخدام القفل العام
            def on_message_wrapper(ws_app, message):
                ...
                if msg_type == 'tick':
                    ...
                    # شرط الدخول المبسَّط للتحقق: يدخل صفقة RISE فوراً بعد التجميع الأول
                    pattern_met = True 
                    ...
                    if pattern_met:
                        ...
                        print(f"📊 [ENTRY CONDITION MET] Forced Entry. Entering {entry_direction} (Initial Trade).")
                    ...
                    # تحديث open_contract_ids هنا لآلية عمل الكود الحالي
                    current_data['open_contract_ids'] = [contract_id] 
                    current_data['contract_profits'] = {} 
                    # 🟢 حفظ معرف العقد في متغير الاسترداد
...
{% if session_data and session_data.is_running %}
    {% set strategy = 'Rise/Fall (5 Ticks) | Entry: Forced RISE Entry (Troubleshooting) | Immediate REVERSED Martingale x' + martingale_multiplier|string + ' (Max ' + max_consecutive_losses|string + ' Losses, Max Step ' + max_martingale_step|string + ')' %}
...
def start_bot():
    ...
    flash(f'Bot started successfully. Strategy: 5 Ticks / Forced RISE Entry (Troubleshooting) / Immediate REVERSED Martingale x{MARTINGALE_MULTIPLIER} (Max {MAX_CONSECUTIVE_LOSSES} Losses, Max Step {MAX_MARTINGALE_STEP}).', 'success')
    return redirect(url_for('index'))
